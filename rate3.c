...
int sortByArrivalPlus(struct ProcStruct proc[], int procCount){  //Sort para ordenar a Struct pelo periodo
    struct ProcStruct temp;
    for (int i = 0; i < procCount-1; ++i) {
        for (int j = i+1; j < procCount; ++j) {
            if(proc[i].periodT > proc[j].periodT && proc[i].waitT>0){
    ...
    return checkAllExecute(proc,procCount); //Retorna só quem pode ser executado!!
}
        ...
        while (should_run == 1) {
            ...
            if (procStruct[0].waitT > 0 || (Time % procStruct[0].periodT == 0)) {
                    ...
                    while (procStruct[0].waitT <= procStruct[checkAllExecute(procStruct, procCount)].periodT && procStruct[0].waitT > 0) {
                        ...
                        for (int i = 1; i < procCount; ++i) {
                            ...
                            if (procStruct[0].periodT > procStruct[i].periodT && Time % procStruct[i].periodT == 0) { //Caso executando encontre um de prioridade MAIOR!
                                ...
                                printf("[%s] for %d units - H\n", procStruct[0].procID,
                                       procStruct[0].execT - procStruct[0].waitT);
                                if (procStruct[0].waitT >
                                    0) { //só printar caso ainda tenha tempo!! Caso não, vai para as proximas condiçoes...
                                ...
                                //verificar novo arrival!! COM WAITTIME
                                       ...
                                       procStruct[0].waitT + Time < periodLimit ||
                                       checkAllExecute(procStruct, procCount) != 0) {
                                //PROB AQUI!!
                                for (int j = 0; j < procCount; ++j) {
                                    if (Time % procStruct[j].periodT == 0 && procStruct[j].waitT > 0 &&
                                        (procStruct[j].waitT !=
                                         procStruct[j].execT)) { //prob quando um proc nunca chega a ser executado! :/
                                ...
                                for (int j = 0; j < procCount; ++j) {
                                    if (Time >= periodLimit && procStruct[j].waitT > 0) {
                                        ...
                                        if ((procStruct[j].execT - killTime < 0 ||
                                             procStruct[j].waitT == procStruct[j].execT) && Time ==
                                                                                            periodLimit) { //ou seja, não tiver sido executado... nem printo!

                                        } else if (Time <= periodLimit && temp !=
                                                                          procStruct[j].procID) { //&& temp != procStruct[j].procID - para evitar prints iguais!!
                                            fprintf(arq, "[%s] for %d units - K\n", procStruct[j].procID,
                                                    procStruct[j].execT -
                                                    killTime); //Resolver o prob de printar 2x no input4.txt
                        ...
                        if (procStruct[0].waitT == 0 && should_run == 1 && Time < periodLimit) {
                            ...
                            if (procStruct[0].execT - mdc(Time, procStruct[0].periodT) != 0 &&
                                Time % procStruct[0].periodT !=
                                procStruct[0].execT) { //pela logica que fiz, esse calculo funcina melhor depois que passam o Tempo dos periodos!! por isso o if
                                if (procStruct[0].execT - mdc(Time, procStruct[0].periodT) < 0) {
                                    ...
                                    fprintf(arq, "[%s] for %d units - F\n", procStruct[0].procID,
                                            procStruct[0].execT -
                                            procStruct[0].finishT); //AQUI O ERRO, quando vem do lost!
                                }
                ...
                should_run = 2;
                       ...
                       checkAllExecute(procStruct, procCount) == 0) { //idle time!! (all .waitT=0 and not waiting)
